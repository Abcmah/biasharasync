...
class PosController extends ApiBaseController
{
    ...
    public function checkMpesaStkPushPayment(Request $request)
    {
        ...
        $transaction = PaymentIntent::where('reference', $checkoutRequestId)->first();

        if (!$transaction) {
            Log::warning(" Transaction not found for checkout: {$checkoutRequestId}");
            return response()->json([
                'success' => false,
        ...
        if (in_array($transaction->status, ['completed', 'failed'])) {
            Log::info(" Transaction final status: {$transaction->status}", [
            ...
            return response()->json([
                ...
                'elapsed_seconds' => $transaction->created_at ? now()->diffInSeconds($transaction->created_at) : 0,
                'message' => $transaction->status === 'completed'
                    ? 'Payment completed successfully'
                    : 'Payment failed: ' . $transaction->result_desc
            ]);
        }
    }
    public function addPosMpesaStkpushPayment(Request $request)
    {
        ...
        $response = (new MpesaService($company->id))->stkPushRequest($validated['phone'], $validated['amount'], 'test');
        // dd($response);
        if (isset($response['ResponseCode']) && $response['ResponseCode'] == '0') {
            $transaction = PaymentIntent::create([
            ...
            Log::error('M-Pesa STK push failed: ', $response);
